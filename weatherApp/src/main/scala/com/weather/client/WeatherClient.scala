...
trait WeatherClient {
  ...
  def getLocation(lat: Double, lon: Double, apiKey: String)
                 (implicit d: EntityDecoder[Task, Location]): Task[Location]
...
private final class WeatherClientImpl(httpClient: HttpClient) extends WeatherClient {
  ...
  override def getLocation(lat: Double, lon: Double, apiKey: String)
                          (implicit d: EntityDecoder[Task, Location]): Task[Location] =
